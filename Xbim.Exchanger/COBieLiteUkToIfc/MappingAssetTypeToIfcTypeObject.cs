...
namespace XbimExchanger.COBieLiteUkToIfc
{
    class MappingAssetTypeToIfcTypeObject : CoBieLiteUkIfcMappings<string, AssetType, IfcTypeObject>
    {
        ...
        {
            Exchanger.SetUserHistory(ifcTypeObject, assetType.ExternalSystem, (assetType.CreatedBy == null) ? null : assetType.CreatedBy.Email, (assetType.CreatedOn == null) ? DateTime.Now : (DateTime)assetType.CreatedOn);
            using (OwnerHistoryEditScope context = new OwnerHistoryEditScope(Exchanger.TargetRepository, ifcTypeObject.OwnerHistory))
            {
                ...
                if (assetType.Spares != null)
                {
                    foreach (var spare in assetType.Spares)
                    {
                        ...
                        relResource.RelatedObjectsType = IfcObjectType.Product;
                    }
                }
                #endregion
