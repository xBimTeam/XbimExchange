...
namespace XbimExchanger.COBieLiteUkToIfc
{
    class MappingFacilityToIfcBuilding : CoBieLiteUkIfcMappings<string, Facility, IfcBuilding>
    {
        ...
        {
            #region Properties
            Exchanger.SetUserHistory(ifcBuilding, facility.ExternalSystem, (facility.CreatedBy == null) ? null : facility.CreatedBy.Email, (facility.CreatedOn == null) ? DateTime.Now : (DateTime)facility.CreatedOn);
            using (OwnerHistoryEditScope ohContext = new OwnerHistoryEditScope(Exchanger.TargetRepository, ifcBuilding.OwnerHistory))
            {
